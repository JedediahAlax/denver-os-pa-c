...
alloc_pt mem_new_alloc(pool_pt pool, size_t size) {
    ...
    if (((float) mgr->used_nodes / mgr->total_nodes) > MEM_NODE_HEAP_FILL_FACTOR) {
        alloc_status resize = _mem_resize_node_heap(mgr);
        if(resize != ALLOC_OK){
            return NULL;
        }
    }
    ...
    if (mgr->used_nodes >= mgr->total_nodes) {
        return NULL;
    }
    ...
    if(mgr->pool.policy == FIRST_FIT){
        node_pt current = mgr->node_heap;
        while((mgr->node_heap[i].allocated != 0 || mgr->node_heap[i].alloc_record.size < size)&& i < mgr->total_nodes){//TODO++++++++
            ++i;
        }
        if(i == mgr->total_nodes){
            return NULL;// check if node found
        }
        insert_node = &mgr->node_heap[i];

    }
        // if BEST_FIT, then find the first sufficient node in the gap index
    else if(mgr->pool.policy == BEST_FIT){
        if(mgr->pool.num_gaps > 0) {
            while (i < mgr->pool.num_gaps && mgr->gap_ix[i + 1].size >= size) {
                if(mgr->gap_ix[i].size == size) break;
                ++i;
            }
        } else return NULL;// check if node found
        insert_node = mgr->gap_ix[i].node;
    }
    ...
    if(insert_node->alloc_record.size - size > 0){
        remaining_size = insert_node->alloc_record.size - size;
    }

    // remove node from gap index TODO
    _mem_remove_from_gap_ix(mgr, size, insert_node);
    ...
    if(remaining_size > 0) {
        ...
        int j = 0;
        while (mgr->node_heap[j].used != 0) {
            ++j;
        }
        node_pt new_gap = &mgr->node_heap[j];
        //   make sure one was found
        if(new_gap != NULL){
            //   initialize it to a gap node
            new_gap->used = 1;
            new_gap->allocated = 0;
            new_gap->alloc_record.size = remaining_size;
        }
        ...
        if(insert_node->next != NULL) insert_node->next->prev = new_gap;
        insert_node->next = new_gap;
        new_gap->prev = insert_node;

        //   add to gap index
        _mem_add_to_gap_ix(mgr, remaining_size, new_gap);
    }

    // return allocation record by casting the node to (alloc_pt)
    return (alloc_pt)insert_node;
}
